...
def create_compressed_video(color_path, output_folder, frame_rate, fourcc, video_name, quality):
    ...
    # Define the codec and create VideoWriter object using MJPEG codec
    cv2_fourcc = cv2.VideoWriter_fourcc(*fourcc)
    video_property = cv2.VideoWriter(video_full_path, cv2_fourcc, frame_rate, size, isColor=True)

    # Set the MJPEG quality property (0 to 100)
    video_property.set(cv2.VIDEOWRITER_PROP_QUALITY, quality)
...
if __name__ == '__main__':
    ...
    fourcc = 'FP90'  # Use 'MJPG' for MJPEG codec
    frame_rate = 30
    video_name = '2.mp4'
    quality = 20
    create_compressed_video(color_path, output_folder, frame_rate, fourcc, video_name, quality)
