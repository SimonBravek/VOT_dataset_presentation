...
def create_compressed_video(color_path, output_folder, frame_rate, fourcc, video_name):
   ...
   size = (int(frame.shape[1] * 0.5), int(frame.shape[0] * 0.5))  # Calculate the new size after resizing

   # Define the codec and create VideoWriter object using H.264 (H264) codec
   ...
   for idx, img in enumerate(list_num_paths):
